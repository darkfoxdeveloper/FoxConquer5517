...
// *                      INFORMATIONS
// * ************************************************************
// * Blowfish (CFB64) class for the library.
// * blowfish.cs
// * 
// * --
// *
...
namespace CO2_CORE_DLL.Security.Cryptography
{
    ...
    {
        private readonly UInt32[] InitP = new UInt32[] {
                0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
                0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
                0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b };

        private readonly UInt32[][] InitS = new UInt32[][] {
            new UInt32[]
        ...
        public const Int32 BLOWFISH_KEY_SIZE = 56;
        public const Int32 BLOWFISH_BLOCK_SIZE = 8;
        ...
        private UInt32[] P = new UInt32[18];
        private UInt32[][] S = new UInt32[4][] { new UInt32[256], new UInt32[256], new UInt32[256], new UInt32[256] };
        ...
        public Blowfish()
        {
            EncryptIV = (Byte*)Kernel.malloc(BLOWFISH_BLOCK_SIZE);
            DecryptIV = (Byte*)Kernel.malloc(BLOWFISH_BLOCK_SIZE);

            Kernel.memset(EncryptIV, 0, BLOWFISH_BLOCK_SIZE);
            Kernel.memset(DecryptIV, 0, BLOWFISH_BLOCK_SIZE);
        }
        ...
        /// <summary>
        /// Generates a key by using less or 56 octets.
        /// </summary>
        public void GenerateKey(Byte* pKey, Int32 Length)
        {
            ...
            P = InitP.Clone() as UInt32[];
            S = InitS.Clone() as UInt32[][];
            EncryptNum = 0;
            DecryptNum = 0;
        ...
        /// <summary>
        /// 'F' function of the Blowfish algorithm.
        /// </summary>
        private UInt32 F(UInt32 Value)
        { return ((S[0][(Value >> 24) & 0xFF] + S[1][(Value >> 16) & 0xFF]) ^ S[2][(Value >> 8) & 0xFF]) + S[3][(Value) & 0xFF]; }

        /// <summary>
        /// 'Encipher' function of the Blowfish algorithm.
        /// Sixteen round encipher of block.
        /// </summary>
        private void Encipher(Byte* pBlock)
        {
            UInt32 LeftValue = ((UInt32*)pBlock)[0];
            UInt32 RightValue = ((UInt32*)pBlock)[1];
            LeftValue ^= P[0];
            RightValue ^= F(LeftValue) ^ P[1]; LeftValue ^= F(RightValue) ^ P[2];
            RightValue ^= F(LeftValue) ^ P[3]; LeftValue ^= F(RightValue) ^ P[4];
            RightValue ^= F(LeftValue) ^ P[5]; LeftValue ^= F(RightValue) ^ P[6];
            RightValue ^= F(LeftValue) ^ P[7]; LeftValue ^= F(RightValue) ^ P[8];
            RightValue ^= F(LeftValue) ^ P[9]; LeftValue ^= F(RightValue) ^ P[10];
            RightValue ^= F(LeftValue) ^ P[11]; LeftValue ^= F(RightValue) ^ P[12];
            RightValue ^= F(LeftValue) ^ P[13]; LeftValue ^= F(RightValue) ^ P[14];
            RightValue ^= F(LeftValue) ^ P[15]; LeftValue ^= F(RightValue) ^ P[16];
            RightValue ^= P[17];
            ((UInt32*)pBlock)[0] = RightValue;
            ((UInt32*)pBlock)[1] = LeftValue;
        }
        ...
        public void SetIVs(Byte* pEncryptIV, Byte* pDecryptIV)
        {
            ...
            Kernel.memcpy(EncryptIV, pEncryptIV, BLOWFISH_BLOCK_SIZE);
            ...
            Kernel.memcpy(DecryptIV, pDecryptIV, BLOWFISH_BLOCK_SIZE);
        ...
        /// <summary>
        /// Encrypts data with the Blowfish algorithm using the Cipher Feedback method.
        /// </summary>
        ...
        /// <summary>
        /// Decrypts data with the Blowfish algorithm using the Cipher Feedback method.
        /// </summary>
        ...
        /// <summary>
        /// Generates a key by using less or 56 octets from the CP-1252 string.
        /// </summary>
        ...
        public void SetIVs(Byte[] EncryptIV, Byte[] DecryptIV)
        {
            ...
            Kernel.assert(EncryptIV.Length == BLOWFISH_BLOCK_SIZE && DecryptIV.Length == BLOWFISH_BLOCK_SIZE);
